 ...
 * @return mixed
 */
function regUser($openid,$name, $icon, $type=1, $token='', $union_id='', $ext_token='', $ccid=0, $test_api = false) {
    if($test_api)
        $api = 'http://121.201.7.97/userreg4h5';
    else
    ...
    $param['token'] = $token;
    $param['ext_token'] = $ext_token;
    $param['nickname'] = urlencode($name) ;
    $param['icon'] = urlencode($icon);
    $param['device_id'] = $_COOKIE['device_id'];
...
function getUser($openid, $unionid, $test_api = false) {
    if($test_api)
        $api = 'http://121.201.7.97/getccid4h5';
    else
