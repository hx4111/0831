...
//alert(window.location.href + ' , 11111');
...
$(function(){
    ...
    var appCcid = getQueryString('ccid'),
		...
		//requestUrl = 'http://121.201.7.97/activityrandom';
	    ...
	    var aj = $.ajax({    
	        ...
	        success : function(response , status , xhr) {
				...
				var data = {
					...
					gift : [{id:0,name:'iPhone6'},{id:1,name:'iPad mini'},{id:2,name:'Apple Watch'},{id:3,name:'Bose运动耳机'},{id:4,name:'小米运动相机'},{id:5,name:'小米电源'},{id:6,name:'小米手环'},{id:7,name:'小米LED灯'},{id:8,name:'10元话费'}],
				...
				$('.balloon-list span').on('click',function(){
					...
					if(useNum == 0){
						...
						if( useNum || data.getGift.id < 0){		//data.getGift.id < 0)
							...
							//中奖的各种延续操作
							$('.succ-con .succ-img img').attr('src','images/gift' + data.getGift.id + '.png');
				...
				var popupSharePanelObj = {
						'title': '壕气来袭，不用割肾免费抽手机',
						'describe' : '单单免费看漫画弱爆了，同时抽点肾六肾表才够劲道！',
						'imageurl' : 'http://cdn.icomicool.cn/m/act/game-shootIOS/images/share-img.jpg?imageMogr2/thumbnail/240x',
						'page_url' : encodeURI('http://m.comicool.cn/act/game-shootIOS/share.html?giftid=' + data.getGift.id + '&giftName='+ data.getGift.name)
					}
				...
				if( data.getGift.id < 0){
					$('.btn-share-lose').on('click',function(){
						...
						callAppFunction("popupSharePanel", popupSharePanelObj);
		               /* try {
		                    comicool.popupSharePanel(popupSharePanelStr);
		                } catch (e) {
		                    window.location.href="http://comicool/popupSharePanel?title="+popupSharePanelObj.title+"&describe="+popupSharePanelObj.describe+"&imageurl="+popupSharePanelObj.imageurl+"&page_url="+popupSharePanelObj.page_url;
		                }*/
