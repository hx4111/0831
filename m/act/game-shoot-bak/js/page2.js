...
$(function(){
	...
	var aj = $.ajax({    
        ...
        data : {
            'ccid' : 16,
            'usertype' : 3,
            'cctoken' : '125cd4f54dbc3577cf58f797010e4275ea2a6be38e8da63359ca47456525b892',
            'deviceid' : '1efb0f505f721cdaaf1ac617d0cf96ce',
            'activityid' : 10,
            'selectnum'	: selectnum
        },    
        type : 'get',
        ...
        success : function(response , status , xhr) {
			...
			$('.balloon-list span').on('click',function(){
				...
				if(useNum == 0){
					...
					if( useNum || data.getGift.id < 0){		//data.getGift.id < 0)
					...
					}else if( data.getGift.id >= 0){		//data.getGift.id >= 0
						//console.log('中');			
						//最后一次射中礼物文字提示
						setTimeout(function(){
						...
						var formUrl = encodeURI('form.html?ccid=' + appCcid + '&giftid=' + data.getGift.id + '&giftName='+ data.getGift.name);
			...
			var popupSharePanelObj = {
				...
				'imageurl' : 'http://cdn.icomicool.cn/m/act/game-shoot/images/share-img3.jpg?imageMogr2/thumbnail/240x',
