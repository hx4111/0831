...
(function() {
	...
	getPostList(0, 5);
	window.onscroll = checkMeetPageBottom;
	...
	document.addEventListener("scroll", function() {
			...
			var discuss = document.getElementById("discuss").offsetTop;
			var publish_betting = getByClass("publish_betting")[0];
			if (scrollTops > (discuss - 700)) {
				publish_betting.style.display = "block";
			} else {
				publish_betting.style.display = "none";
			}
		})
		//	 点赞+1
	$('.icon-zan').click(function() {
		...
		if (getCookie('flag') == "true") {
			...
			$.get("http://comicool.cn/simple_vote.php?sel=2");
		...
		$('#zhan').animate({
			...
			function() {
				...
				$(this).fadeOut(50000, function() {
					$.fn.ferroMenu.toggleMenu(newNav)
				})
			});
	});
