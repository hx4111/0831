...
for (item in filling[cycle].pic) {
	if(filling[cycle].pic[item][2]){
	hodgepodge += '<li class="go" data-ccid="'+filling[cycle].pic[item][1]+'" data-epid="'+filling[cycle].pic[item][2]+'"><img src="' + filling[cycle].pic[item][0] + '" alt="' + getShare(cycle, "alt", 0) + '"></li>';
	}else{
		hodgepodge += '<li class="go" data-ccid="'+filling[cycle].pic[item][1]+'"><img src="' + filling[cycle].pic[item][0] + '" alt="' + getShare(cycle, "alt", 0) + '"></li>';	
	}
	
}
$("#check_hodgepodge").html(hodgepodge);
...
//				判断有没有之前点赞
var flag_fifty_seven = checkinGlobal.getCookie("flag_fifty_seven");
var flag_fifty_eight = checkinGlobal.getCookie("flag_fifty_eight");
if (flag_fifty_seven) {
	$("#oppose i").addClass("current");
} else if (flag_fifty_eight) {
...
$("#zan ul").on("click", function() {
	var flag_fifty_seven = checkinGlobal.getCookie("flag_fifty_seven");
	var flag_fifty_eight = checkinGlobal.getCookie("flag_fifty_eight");
	//	checkinGlobal.clearCookie("flag_fifty_seven")
	//	checkinGlobal.clearCookie("flag_fifty_eight")
	var hasPraised = $(this).attr("id");
	if (flag_fifty_seven || flag_fifty_eight) {
		...
		if (hasPraised == "oppose") {
			...
			$("#oppose i").addClass("current");
			$.get("http://comicool.cn/simple_vote.php?sel=57");
			checkinGlobal.setCookie("flag_fifty_seven", "true", 30);
		} else {
			var num = $("#support span").html();
			$("#support span").html(parseInt(num) + 1);
			$("#support i").addClass("current");
			$.get("http://comicool.cn/simple_vote.php?sel=58");
			checkinGlobal.setCookie("flag_fifty_eight", "true", 30);
		}
	}
})
$.ajax({
	...
	success: function(data) {
		$("#oppose span").html(data.results[57]);
		$("#support span").html(data.results[58]);
	}
})
...
window.onload=function(){
	...
	imageUrl: getShare(cycle, "share", "imageUrl")?getShare(cycle, "share", "imageUrl"):getShare(3, "share", "imageUrl"),
...
(function() {
	...
	var initLogin = function(obj) {
		...
		if (CONFIG.isAndroidApp && isSetCalender) {
			return false;
		}
		getCheckHistory();
	};
