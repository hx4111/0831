...
(function() {
	...
	var renderBaseInfo = function(res) {
		//banner
		res.banner_list = [{
			"banner_url": "http://m.comicool.cn/betting_result.html?league_order=48",
			"banner_cover": "http://cdn.icomicool.cn/m/images/betting/banner25.png"
		}];
		if (res.banner_list && res.banner_list.length) {
			render('tmpl-banner', res);
			TouchSlide({
				//				slideCell:"#h-slider",
				titCell: ".hd ul",
				mainCell: ".bd ul",
		...
		try {
			...
			var matchError = '<div><img src="images/betting/errimg.jpg" width="100%"></div>';
			$(".betting").setAttribute("style", "height:11rem");
	...
	function getBaseInfo(obj) {
		...
		jsonp({
			...
			// url: 'http://192.168.10.54:8888/betInfolist4h5',
			data: data,
	...
	function countDown(timestamp) {
		var box = $('.time_betting');
		var spans = $$('span', box);
		var dayCont = spans[0];
		var hourCont = spans[1];
		var minuteCont = spans[2];
		var delay = +minuteCont.dataset.delay + 1;
