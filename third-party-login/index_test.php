...
class  Common
{
    ...
    public function login()
    {
        ...
        if (IN_WEIXIN) {
            global $weObj;
            session('uid', null);
            $weixin_url = $weObj->getOauthRedirect(SITE_PATH . '/index_test.php?c=callback&type=wx', 'wx');
            echo $weixin_url;
            sleep(5);
            http_redirect($weixin_url);
        } else {
            $sae_url = $sae->getAuthorizeURL(SITE_PATH . '/index_test.php?c=callback&type=wb');
            if (CLI_TYPE == "weibo") {
            ...
            if(CLI_TYPE == 'web') {
                $redirect_uri = urlencode(SITE_PATH . '/index_test.php?c=callback&type=wxopen&cli=pc');
        ...
        switch($auto_login) {
            ...
            case "qq":
                http_redirect(SITE_PATH . "/index_test.php?goqq=1");
                break;
            ...
            case "pptv":
                ...
                $data = [
                    ...
                    "response_type" => "code",
                    "state" => "pptv",
                    "redirect_uri" => SITE_PATH . '/index_test.php?c=callback',
                ];
    ...
    public function callback()
    {
        ...
        if($_POST['state'] == "pptv" || $login_type == "pptv") {
            $data = [
                ...
                'redirect_uri'=> SITE_PATH . "/index_test.php?c=callback&type=pptv",
                'code'=>$_POST['code'],
                'format'=>'json',
            ];
        ...
        if ((IN_WEIXIN && $type == 1) || ($login_type == 'wx') || $login_type == "wxopen") {
            $ret = getUser($wx_user['openid'], $wx_user['unionid'], true);
            if (!$ret['ccid'])
                $need_reg = true;
        ...
        if ($need_reg)
            $ret = regUser($openId, $nickname, $avatar, $type  , $ac_token, $unionid, $refresh_token,  session('tg_ccid'), true, session('device_id'), $phone);
        ...
        if ($ret['ccid']) {
            ...
            if($referer = session('referer')) {
                ...
                if(session('callback_type') == "jsonp") {
                    ...
                    $qstr = "key=".$key;
                    $qstr = "getdatauri=".urlencode(SITE_PATH . "/index_test.php?c=get_data_by_jsonp&key=".$key);
    ...
    public function get_data_by_jsonp() {
        ...
        if(file_exists($path)) {
            $result["data"] = 1;
            $result["data"] = unserialize(file_get_contents($path));
        }
    ...
    private  function initQQConnect() {
        ...
        \Ext\QQConnect::$callback = SITE_PATH.'/index_test.php?c=callback&type=qq';
